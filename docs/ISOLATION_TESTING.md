...
- A lease requests an **ordered isolation level**: `shared` < `sandboxed` < `vm`
  ...
  Omitted → `shared` (unchanged legacy behavior).
- **Capability = policy ∩ host.** A host advertises the *effective* set = the operator's allowed
  ...
  on Linux **or** a Windows daemon). Advertised on `GET /images` under `isolation:{supported,default}`.
...
- With `WISP_CONFIG` allowing `[shared,sandboxed,vm]`, `GET /images` advertised `[shared,vm]` —
  `sandboxed` dropped by the intersection (no runsc on a Windows daemon). `sandboxed` → 400 "not allowed".
...
was shipped from a local clone via an **S3 presigned URL** (the repo is private) and built on the box.
...
- **Orphan reconcile across a real restart.** Unit-tested (`internal/server/reconcile_test.go`); an
  E2E check would kill/restart `wispd` with live labeled containers and confirm they are re-tracked/reaped.
